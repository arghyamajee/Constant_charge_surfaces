...
int M, N, n, k, l;
double e1, s1, I1, e2, s2, I2, dx, dz, l_B, phi_D;
double A, B, C, D, E, F;
...
int main (int args, char *arg[])
{
   if(args!=20)
   {
      cerr<<"Program1 <e1> <e2> <s1> <s2> <I1> <I2> <dx> <dz> <phi_D> <M> <N> <file_phi_low> <file_phi_up> <tol> <iter_max> <iter_write> <l_B> <alpha> <n>" <<endl;
      return 5;
   }
   e1 = atof(arg[1]);
   e2 = atof(arg[2]);
   ...
   int iter_write = atoi(arg[16]);
   l_B = atof(arg[17]);
   double alpha = atof(arg[18]);
   n = atoi(arg[19]);
